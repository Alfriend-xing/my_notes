...
排序：暴力破解的时间复杂的是n^2，会超时。所以先对数组按值排序降低复杂度。保存降序排列后的索引值，遍历之。则每次得到的索引在数组中的值都大于之前的，只需比较宽度(i2-i1)即可。如果i2-i1为负数直接不考虑，为使i2-i1为最宽值，需要在遍历时保存已遍历过的i的最小值，虽然就算保存的遍历过的最小值，后续遍历时还是可能遇到负宽度，不过没关系，把负宽度时的i2作为已遍历的i的最小值保存即可，最大宽度还是取历史最大值和当前i2-i1值里最大的。
...
class Solution {
    public int maxWidthRamp(int[] A) {
        ...
        for (int i: B) {
            ...
            //第一次进入遍历时，索引i在数组中对应的值最小，不可能作为i2计算宽度，让其与索引最大值N相减，保持目前最大宽度ans为0
            //之后进入遍历的i2会与之前遍历过的最小索引值相减获得每个索引能组成的最大坡宽度，再与历史最大宽度相比取最大值
            
            m = Math.min(m, i);
            //首次进入遍历的i会与索引可能出现的最大值N相比，m肯定会取i的值，保证之后每次计算宽度所用的m都是之前遇到过的最小的索引
        }
